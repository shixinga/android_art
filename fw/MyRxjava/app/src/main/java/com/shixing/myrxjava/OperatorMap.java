...
public class OperatorMap<T,R> implements Operator<R,T>  {
     ...
     * 到底是如何将  羊变成铁   这里不管
     */
    Func<? super T,? extends R> transform;

    public OperatorMap(Func<? super T, ? extends R> transform) {
    ...
    private class MapSubscrble<T,R> extends Subscrible<T>
    {
        ...
        public MapSubscrble(Subscrible<? super R> actual, Func<? super T, ? extends R> transform) {
            this.actual = actual;
            this.transform = transform;
        }
