 ...
 */
public class ChatAdapterOptimization extends ArrayAdapter<Chat> {
    public ChatAdapterOptimization(Context context, ArrayList<Chat> chats) {
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        if (view == null) {
            view = LayoutInflater.from(getContext()).inflate(
                    R.layout.chat_item_s2_optimization, parent, false);
        }
...
//        chat_author_avatar.setBackgroundColor(chat.getAuthor().getColor());
        ...
        //if有图片就加载图片，if没有图片就给默认的图片(为transparent透明的--节省绘制次数)
        if (chat.getAuthor().getAvatarId() != 0) {
            Picasso.with(getContext()).load(chat.getAuthor().getAvatarId()).into(
                    chat_author_avatar);
